...
describe("Page when no label is selected and no query is input", () => {
  ...
  describe("and the query is not a match", () => {
    ...
    await act(async () => {
      /* eslint-disable @typescript-eslint/no-explicit-any */
      render(<Home searchParams={Promise.resolve({ query: "bla" } as any)} />);
    });
  });
  ...
  describe("and the query is a match", () => {
      beforeEach(async () => {
    await act(async () => {
      render(<Home searchParams={Promise.resolve({ query: "togglable" } as any)} />);
    });
  });
  ...
  it("renders all cards", () => { // fail expected
    const cards = screen.getAllByRole("card");
